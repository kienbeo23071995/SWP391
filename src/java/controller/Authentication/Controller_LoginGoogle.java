    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            if (code == null || code.isEmpty()) {
                ...
                DAOAccount accDAO = new DAOAccount();
                Account acc = accDAO.getAccountByEmail(googlePojo.getEmail());
                if (acc == null) {
                    DAOAccount dQ = new DAOAccount();
                    List<Secure_Questions> lsQ = dQ.getListQuestion();
                    List<Account_Role> lsR = dQ.getListRole();
                    request.getSession().setAttribute("disabled", true);
                    request.setAttribute("email", googlePojo.getEmail());
                    request.setAttribute("lsR", lsR);
                    request.setAttribute("lsQ", lsQ);
                    request.setAttribute("warning", "Bạn vui lòng điền đầy đủ thông tin bên dưới để hoàn tất");
