 ...
 * @author kienb
 */
public class Controller_Change_Password extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet Controller_Change_Password</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet Controller_Change_Password at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (oldPassword.trim().isEmpty() || newPassword.trim().isEmpty() || confirmPassword.trim().isEmpty()) {
            ...
            if (check.checkExistPassword(email, oldPassword)) {
                ...
                request.setAttribute("errorOld", "Incorrect password or email!");
                request.getRequestDispatcher("changepass.jsp").forward(request, response);
            }
        }
    }
